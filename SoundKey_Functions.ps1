...
function Get-SortedAudioFiles {
    ...
    $audioFiles = Get-ChildItem -Path $directoryPath -File -Include *.wav,*.mp3 | Sort-Object Name

    # If no files found at top level, search recursively in all subdirectories.
    if (-not $audioFiles) {
        $audioFiles = Get-ChildItem -Path $directoryPath -File -Include *.wav,*.mp3 -Recurse | Sort-Object Name
    }
